...
export default {
  ...
  execute: async (interaction: ModalSubmitInteraction) => {
    ...
    const courses = await new HttpUtils().get(Routes.GET_COURSES_BY_GUILD, guild_uuid!)
      .catch(logger.error);

    const courses_name = courses.data.map((course: any) => course.name.toLocaleLowerCase());

    if(courses_name.includes(name.toLowerCase())) {
      await interaction.reply({content: 'Ce nom de formation est déjà utilisé', ephemeral: true});
      return;
    }
    ...
    const channelStock = new HttpUtils().get(Routes.GET_CHANNELS_STOCK, guild_uuid!);
